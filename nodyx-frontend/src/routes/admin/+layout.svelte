<script lang="ts">
	import { page } from '$app/state'
	import type { LayoutData } from './$types'
	import NodyxVersionBadge from '$lib/components/NodyxVersionBadge.svelte'
	import { t } from '$lib/i18n'

	const tFn = $derived($t)

	let { children, data }: { children: any; data: LayoutData } = $props()

	// iconPath : SVG outline 24x24 (heroicons/lucide), rendu monochrome via
	// stroke currentColor. Pas d'emoji dans les labels système (référence
	// design admin : Linear / Vercel / Stripe, un seul accent).
	type NavItem = { href: string; labelKey: string; iconPath: string; exact?: boolean; soon?: boolean }
	type NavGroup = { sectionKey: string; items: NavItem[] }

	const nav: NavGroup[] = [
		{
			sectionKey: 'anav.sec_community',
			items: [
				{ href: '/admin',             labelKey: 'anav.lbl_dashboard',    exact: true, iconPath: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z' },
				{ href: '/admin/members',     labelKey: 'anav.lbl_members',      iconPath: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
				{ href: '/admin/grades',      labelKey: 'anav.lbl_grades',       iconPath: 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z' },
				{ href: '/admin/categories',  labelKey: 'anav.lbl_categories',   iconPath: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z' },
				{ href: '/admin/moderation',  labelKey: 'anav.lbl_moderation',   iconPath: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
				{ href: '/admin/octoguard',   labelKey: 'anav.lbl_octoguard',    iconPath: 'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z' },
				{ href: '/admin/trik/channels', labelKey: 'anav.lbl_trik',       iconPath: 'M6.5 6.5l11 11M13 3l3 3-8 8-4 1 1-4 8-8z' },
				{ href: '/admin/tags',        labelKey: 'anav.lbl_tags',         iconPath: 'M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z' },
				{ href: '/admin/audit-log',   labelKey: 'anav.lbl_journal',      iconPath: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
			],
		},
		{
			sectionKey: 'anav.sec_communication',
			items: [
				{ href: '/admin/channels/text',  labelKey: 'anav.lbl_channels_text',  iconPath: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
				{ href: '/admin/channels/voice', labelKey: 'anav.lbl_channels_voice', iconPath: 'M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z' },
			],
		},
		{
			sectionKey: 'anav.sec_content',
			items: [
				{ href: '/admin/homepage',         labelKey: 'anav.lbl_homepage',     iconPath: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
				{ href: '/admin/homepage/builder', labelKey: 'anav.lbl_grid_builder', iconPath: 'M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z' },
				{ href: '/admin/widgets',          labelKey: 'anav.lbl_widgets',      iconPath: 'M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z' },
				{ href: '/admin/extensions',       labelKey: 'anav.lbl_extensions',   iconPath: 'M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z' },
				{ href: '/admin/media',            labelKey: 'anav.lbl_media',  iconPath: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' },
				{ href: '/admin/assets',           labelKey: 'anav.lbl_assets',       iconPath: 'M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4' },
				{ href: '/admin/music',            labelKey: 'anav.lbl_music',        iconPath: 'M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2z' },
				{ href: '/admin/garden',           labelKey: 'anav.lbl_garden',       iconPath: 'M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z' },
			],
		},
		{
			sectionKey: 'anav.sec_platform',
			items: [
				{ href: '/admin/modules', labelKey: 'anav.lbl_modules', iconPath: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
			],
		},
		{
			sectionKey: 'anav.sec_integrations',
			items: [
				{ href: '/admin/streamer-hub', labelKey: 'anav.lbl_streamer_hub', iconPath: 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z' },
			],
		},
		{
			sectionKey: 'anav.sec_instance',
			items: [
				{ href: '/admin/announcements', labelKey: 'anav.lbl_announcements',      iconPath: 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z' },
				{ href: '/admin/settings',      labelKey: 'anav.lbl_settings',    iconPath: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
				{ href: '/admin/status',        labelKey: 'anav.lbl_status', iconPath: 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9' },
				{ href: '/admin/backups',       labelKey: 'anav.lbl_backups',   iconPath: 'M4 7v10c0 2 3.582 3 8 3s8-1 8-3V7M4 7c0 2 3.582 3 8 3s8-1 8-3M4 7c0-2 3.582-3 8-3s8 1 8 3' },
				{ href: '/admin/ai',            labelKey: 'anav.lbl_neural', iconPath: 'M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z' },
			],
		},
	]

	const isActive = (href: string, exact = false) =>
		exact
			? page.url.pathname === href
			: page.url.pathname.startsWith(href)

	// TIROIR MOBILE. Mesure du 17/08 a 390px : la barre laterale `w-56` (224px)
	// restait affichee, il ne restait donc que 166px pour le contenu. Le titre de
	// page etait coupe, et les tableaux amputes (5920 elements hors ecran sur la
	// page Membres, sans aucun defilement possible).
	let menuOuvert = $state(false)

	// Fermer a chaque navigation, sinon le tiroir masque la page qu'on vient
	// d'ouvrir depuis le tiroir lui-meme.
	$effect(() => {
		void page.url.pathname
		menuOuvert = false
	})
</script>

<div class="flex flex-1 min-h-0">

	<!-- Voile, sous `lg` uniquement : le tiroir se ferme au clic a cote. -->
	{#if menuOuvert}
		<div class="lg:hidden fixed inset-0 z-30 bg-black/60 backdrop-blur-sm"
		     role="button" tabindex="-1"
		     onclick={() => (menuOuvert = false)}
		     onkeydown={(e) => e.key === 'Escape' && (menuOuvert = false)}
		     aria-label={tFn('anav.close_menu')}></div>
	{/if}

	<!-- ── Sidebar ──────────────────────────────────────────────────────────── -->
	<aside class="w-56 shrink-0 border-r border-zinc-800/80 bg-zinc-950 flex flex-col
	               max-lg:fixed max-lg:inset-y-0 max-lg:left-0 max-lg:z-40 max-lg:overflow-y-auto
	               max-lg:transition-transform max-lg:duration-200
	               {menuOuvert ? 'max-lg:translate-x-0' : 'max-lg:-translate-x-full'}">

		<!-- Header -->
		<div class="h-14 flex items-center gap-2.5 px-4 border-b border-zinc-800/80">
			<div class="w-6 h-6 rounded-md bg-indigo-600 flex items-center justify-center shrink-0">
				<svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
					<path d="M12 3l8 4.5v9L12 21l-8-4.5v-9L12 3z"/>
				</svg>
			</div>
			<div class="min-w-0">
				<div class="text-[13px] font-semibold text-zinc-100 truncate leading-tight">{tFn('anav.admin_title')}</div>
				<div class="text-[11px] text-zinc-500 truncate leading-tight">{data.communityName ?? 'Nodyx'}</div>
			</div>
		</div>

		<!-- Retour au forum : en tête de sidebar, pas en pied. Une sidebar longue
		     (beaucoup de sections) pousse le pied de page hors champ, il faut
		     dérouler toute la page pour l'atteindre (retour Jonathan, 2026-08-08). -->
		<div class="p-2 border-b border-zinc-800/80">
			<a
				href="/"
				class="flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-[13px] text-zinc-500
				       hover:text-zinc-100 hover:bg-zinc-900 transition-colors"
			>
				<svg class="w-4 h-4 text-zinc-600" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
					<path d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
				</svg>
				<span>{tFn('anav.back_to_forum')}</span>
			</a>
		</div>

		<!-- Nav -->
		<nav class="flex-1 overflow-y-auto py-3 px-2">
			{#each nav as group}
				<div class="mb-3.5">
					<p class="px-2.5 mb-1 text-[10px] font-semibold text-zinc-600 uppercase tracking-[0.08em]">
						{tFn(group.sectionKey)}
					</p>
					{#each group.items as item}
						{@const active = isActive(item.href, item.exact)}
						<a
							href={item.href}
							class="relative flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-[13px] transition-colors mb-px
							       {active
							         ? 'bg-zinc-800/80 text-zinc-100'
							         : 'text-zinc-400 hover:text-zinc-100 hover:bg-zinc-900'}
							       {item.soon ? 'opacity-60 pointer-events-none' : ''}"
						>
							{#if active}
								<span class="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 rounded-full bg-indigo-400"></span>
							{/if}
							<svg class="w-4 h-4 shrink-0 {active ? 'text-indigo-300' : 'text-zinc-500'}" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
								<path d={item.iconPath}/>
							</svg>
							<span class="flex-1 truncate">{tFn(item.labelKey)}</span>
							{#if item.soon}
								<span class="text-[10px] text-zinc-600 font-medium shrink-0">{tFn('anav.soon')}</span>
							{/if}
						</a>
					{/each}
				</div>
			{/each}
		</nav>

		<!-- Footer -->
		<div class="border-t border-zinc-800/80 p-2">
			<NodyxVersionBadge
				version={data.updateCheck?.current_version ?? (data as any).nodyxVersion ?? 'unknown'}
				variant="admin"
			/>
		</div>
	</aside>

	<!-- ── Main content ─────────────────────────────────────────────────────── -->
	<div class="flex-1 flex flex-col min-w-0">

		<!-- Update banner -->
		{#if data.updateCheck?.has_update}
			<div class="bg-zinc-900 border-b border-zinc-800 px-6 py-2 flex items-center justify-between gap-4 shrink-0">
				<div class="flex items-center gap-2.5 text-[13px]">
					<span class="w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0"></span>
					<span class="text-zinc-200 font-medium">{tFn('anav.update_available')}</span>
					<span class="text-zinc-500 tabular-nums">v{data.updateCheck.current_version} → v{data.updateCheck.latest_version}</span>
				</div>
				{#if data.updateCheck.release_url}
					<a
						href={data.updateCheck.release_url}
						target="_blank"
						rel="noopener noreferrer"
						class="text-xs font-medium text-zinc-200 hover:text-white border border-zinc-700 hover:border-zinc-500 px-2.5 py-1 rounded-md transition-colors shrink-0"
					>
						{tFn('anav.release_notes')}
					</a>
				{/if}
			</div>
		{/if}

		<!-- Top bar -->
		<header class="h-14 border-b border-zinc-800/80 bg-zinc-950/60 flex items-center justify-between px-4 lg:px-6 shrink-0 gap-2">
			<button type="button" onclick={() => (menuOuvert = true)}
			        class="lg:hidden shrink-0 -ml-2 w-11 h-11 flex items-center justify-center rounded-lg
			               text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors"
			        aria-label={tFn('anav.open_menu')}>
				<svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" viewBox="0 0 24 24">
					<path d="M4 6h16M4 12h16M4 18h16"/>
				</svg>
			</button>
			<nav class="text-[13px] text-zinc-500 flex items-center gap-1.5 min-w-0">
				<a href="/" class="hover:text-zinc-300 transition-colors">{tFn('anav.forum')}</a>
				<svg class="w-3 h-3 text-zinc-700" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg>
				<span class="text-zinc-300">{tFn('anav.admin_title')}</span>
			</nav>
			<div class="flex items-center gap-2 text-xs">
				<span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
				<span class="text-zinc-400"><span class="text-zinc-200 font-medium tabular-nums">{data.stats?.online ?? 0}</span> {tFn('anav.online')}</span>
			</div>
		</header>

		<!-- Page content -->
		<main class="flex-1 overflow-auto p-6">
			{@render children()}
		</main>
	</div>
</div>
